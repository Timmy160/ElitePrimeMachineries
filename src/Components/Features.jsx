...
const Features = () => {
  ...
  return (
    <section
      id="features"
      className="py-20 md:py-32 px-6 md:px-12 lg:px-20 bg-white relative overflow-hidden"
      style={{ fontFamily: '"Space Grotesk", "Inter", sans-serif' }}
    >
      {/* Background glows - made subtle for white */}
      <div className="absolute top-1/2 left-0 w-96 h-96 bg-violet-200/30 rounded-full blur-3xl -translate-y-1/2" />
          ...
          {features.map((feature, index) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative overflow-hidden rounded-2xl border border-violet-200 hover:border-cyan-400/50 transition-all duration-500 ${
              ...
              >
                ...
                <div className={`relative z-10 p-8 h-full flex flex-col ${isLarge ? "md:flex-row md:items-center gap-8" : ""}`}>
                  ...
                  <div className={`flex-shrink-0 ${isLarge ? "md:w-20" : "mb-6"}`}>
                    <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-violet-100 to-cyan-100 flex items-center justify-center border border-violet-200 group-hover:border-cyan-400/50 transition-colors duration-300">
                  ...
                  {/* Content */}
                  <div className="flex-1">
                    <h3 className={`font-bold text-slate-900 mb-3 group-hover:text-cyan-600 transition-colors duration-300 ${isLarge ? "text-2xl" : "text-xl"}`}>
                      {feature.title}
                    </h3>
                    ...
                    {isLarge && (
                      <motion.div 
                        className="mt-4 flex items-center gap-2 text-cyan-600 font-semibold text-sm uppercase tracking-wider opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                      >
        ...
        >
          {[
            ...
            <div key={i} className="text-center">
              <div className="text-3xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-violet-600 to-cyan-500 mb-1">
                {stat.value}
              </div>
